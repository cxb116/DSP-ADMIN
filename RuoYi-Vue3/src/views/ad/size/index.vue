...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["sizeRef"].validate(valid => {
    if (valid) {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除样式尺寸编号为"' + _ids + '"的数据项？').then(function() {
    return delSize(_ids)
  }).then(() => {
    getList()
